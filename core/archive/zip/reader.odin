...
read_directory_end :: proc(
    ...
    // look for directory_end_signature in the last 1K, then in the last 65K
    buf: []byte
	defer delete(buf)
    ...
    if d.directory_records == 0xffff || d.directory_size == 0xffff || d.directory_offset == 0xffffffff {
...
read_directory_header :: proc(f: ^File, r: io.Reader) -> Reader_Error {
    ...
    d := make([]byte, filename_len + extraLen + comment_len)
	defer delete(d)
    if _, err := io.read_full(r, d); err != nil {
        return err
    }
    f.name = strings.clone(string(d[:filename_len]))
    f.extra = bytes.clone(d[filename_len:filename_len + extraLen])
    f.comment = strings.clone(string(d[filename_len + extraLen:]))
    ...
    parse_extras: for extra := f.extra; len(extra) >= 4; {     // need at least tag and size
